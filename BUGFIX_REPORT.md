...
## 15. ปรับรูปแบบการแสดงผลค่าเปอร์เซ็นต์ทุกหน้าจอเป็นทศนิยม 2 ตำแหน่ง (`v2026.0924.1935`)

### 15.1 รายการแก้ไขและปรับปรุง
1. **บั๊ก #45 (Data Precision & Inconsistency) — การแสดงผลเปอร์เซ็นต์บางจุดเป็นจำนวนเต็มหรือทศนิยม 1 ตำแหน่ง ไม่สม่ำเสมอทั่วทั้งแอปพลิเคชัน (`DashboardView.jsx`, `DepositReportView.jsx`, `DepositReportModal.jsx`)**:
   - **ปัญหา**: 
     - หน้าสถิติ (`DashboardView.jsx`) เดิมแสดงเปอร์เซ็นต์ตามที่ backend ส่งมา ซึ่งถูกปัดเศษเหลือ 1 ตำแหน่ง (`round(..., 1)`) หรือแสดงเป็น `0%` เมื่อเป็นศูนย์
     - หน้ารายงานสถานะ (`DepositReportView.jsx`) และหน้าต่างโมดัล (`DepositReportModal.jsx`) เดิมใช้ `Math.round(...)` ทำให้ได้เลขจำนวนเต็มไม่มีทศนิยม เช่น `97%` แทนที่จะเป็นตัวเลขละเอียด
   - **แก้ไข**: 
     - เพิ่มฟังก์ชัน `formatPct(val)` ใน `DashboardView.jsx` แปลงค่าเป็นทศนิยม 2 ตำแหน่งด้วย `.toFixed(2)` เสมอ (เช่น `96.80%`, `16.22%`, `0.00%`) ทั้งบนการ์ดสรุปผล 4 ใบ, Tooltip แผนที่, แผงข้อมูลจังหวัด, ตารางจัดอันดับจังหวัด (%สำเร็จ), และแท่งแสดงสาเหตุการส่งคืน/นำจ่ายไม่สำเร็จ
     - ปรับการคำนวณ `receivedRate`, `inTransitRate`, `deliveredRate`, และ `returnedRate` ใน `DepositReportView.jsx` และ `DepositReportModal.jsx` เป็น `((count / total) * 100).toFixed(2)`
2. **บั๊ก #46 (Progress Precision) — เปอร์เซ็นต์ความคืบหน้าการแปลงไฟล์ไม่สม่ำเสมอ (`App.jsx`, `api.js`)**:
   - **แก้ไข**: ปรับการคำนวณเปอร์เซ็นต์ความคืบหน้าใน `convertPdfs` และข้อความเริ่มต้นใน `App.jsx` ให้แสดงเป็นทศนิยม 2 ตำแหน่ง (`0.00%` ถึง `100.00%`)
3. **Backend Precision Alignment (`api/index.py`)**:
   - ปรับการคำนวณเปอร์เซ็นต์ในฟังก์ชันสรุปผลแดชบอร์ด (`delivered_pct_of_concluded`, `failed_pct_of_concluded`, `received_pct_of_total`, `in_transit_pct_of_total`, `pct_of_failed`, และ `success_rate`) จาก `round(..., 1)` เป็น `round(..., 2)`

### 15.2 ผลลัพธ์การทดสอบและการ Build
- ตรวจสอบความถูกต้องของสคริปต์ Python AST: ผ่าน 100%
- ทดสอบระบบป้องกันสระวรรณยุกต์และตัวอักษรผิดเพี้ยน (`encoding_guard.mjs`): ผ่าน 56 ไฟล์ 100% (0 mojibake)
- การคอมไพล์ Production Bundle (`npm run build`): สำเร็จไร้ข้อผิดพลาด
- ซิงก์เลขเวอร์ชัน 6 ตำแหน่งตรงกัน: `v2026.0924.1935`

---

## 16. ซิงก์การแสดงผลการ์ดสรุปยอดหน้าสถิติให้ตรงกับหน้ารายงานสถานะ 100% (`v2026.0924.1945`)

### 16.1 รายการแก้ไขและปรับปรุง
1. **บั๊ก #47 (UI & Metric Hierarchy Consistency) — ความไม่สอดคล้องของตำแหน่ง ข้อความ รูปแบบ ค่า และสไตล์การ์ดสรุปยอดระหว่างหน้า "สถิติ" และหน้า "รายงานสถานะ" (`DashboardView.jsx`, `DashboardView.css`)**:
   - **ตำแหน่งการแสดงผล (Card Order)**: ปรับการเรียงลำดับการ์ด KPI ให้ตรงกับหน้ารายงานสถานะ: `รับฝากแล้ว` -> `อยู่ระหว่างการนำจ่าย` -> `นำจ่ายสำเร็จ` -> `ส่งคืน` -> `รายการทั้งหมด` (ย้ายยอดรวมมาไว้ท้ายสุดเพื่อสะท้อนลำดับกระบวนการนำจ่าย)
   - **ข้อความกำกับ (Labels)**: เปลี่ยนคำกำกับการ์ดใบที่ 4 จาก "ส่งคืน / ไม่สำเร็จ" เป็น "ส่งคืน" และใบที่ 5 จาก "รับฝาก (ทั้งหมด)" เป็น "รายการทั้งหมด"
   - **ค่าและฐานการคำนวณ (Values & Precision)**: ปรับการคำนวณ `receivedRate`, `inTransitRate`, `deliveredRate`, และ `returnedRate` ให้เทียบจาก `totalItems` (ยอดรับฝากทั้งหมด) เสมอ และแสดงผลด้วยทศนิยม 2 ตำแหน่ง (`.toFixed(2)`)
   - **รูปแบบการแสดงผล (Format)**: เปลี่ยนจากการแสดงผลแบบแยกข้อความยาว เช่น `12 (16.20% ของทั้งหมด)` เป็นรูปแบบกะทัดรัด สะอาดตา ตามหน้ารายงานสถานะ เช่น `12 (16.22%)` และการ์ดยอดรวมแสดง `{totalItems} ฉบับ`
   - **สไตล์ สี และไอคอน (Styles & Icons)**:
     - นำเข้าคลาสสีตัวเลข: `.text-blue` (รับฝากแล้ว), `.text-amber` (อยู่ระหว่างนำจ่าย), `.text-emerald` (นำจ่ายสำเร็จ), และ `.text-rose` (ส่งคืน) ทั้ง Light Mode และ Dark Mode
     - ซิงก์ชุดไอคอน SVG และคลาสพื้นหลังไอคอน (`icon-received-deposit`, `icon-transit`, `icon-delivered`, `icon-returned`, `icon-total`) ตรงกับหน้ารายงานสถานะ 100%

### 16.2 ผลลัพธ์การทดสอบและการ Build
- ตรวจสอบความถูกต้องของสคริปต์ Python AST: ผ่าน 100%
- ทดสอบระบบป้องกันสระวรรณยุกต์และตัวอักษรผิดเพี้ยน (`encoding_guard.mjs`): ผ่าน 56 ไฟล์ 100% (0 mojibake)
- การคอมไพล์ Production Bundle (`npm run build`): สำเร็จไร้ข้อผิดพลาด
- ซิงก์เลขเวอร์ชัน 6 ตำแหน่งตรงกัน: `v2026.0924.1945`

---

## 17. ตรวจจับและดึงสาเหตุการส่งคืนพัสดุจากสถานะจริงก่อน "ปณ.ปลายทางส่งคืน" (`v2026.0924.2005`)

### 17.1 รายการแก้ไขและปรับปรุง
1. **บั๊ก #48 (Root Cause Obfuscation) — พัสดุที่ถูกส่งคืนแสดงสาเหตุเป็น "อื่น ๆ" หรือแสดงเพียงขั้นตอนการขนส่งกลับ "ปณ.ต้นทางส่งคืนบริษัท" แทนที่จะเป็นสาเหตุข้อยกเว้นจริงที่เกิดขึ้น (`api/index.py`, `TrackingTimelineModal.jsx`)**:
   - **รากเหง้าของปัญหา**: ในระบบของไปรษณีย์ไทย เมื่อการนำจ่ายที่ไปรษณีย์ปลายทางล้มเหลว เจ้าหน้าที่จะบันทึกสถานะข้อยกเว้นก่อน (เช่น `ย้าย / ไม่ทราบที่อยู่ใหม่`, `บ้านปิด`) จากนั้นจึงบันทึกสถานะ `ปณ.ปลายทางส่งคืน` และส่งพัสดุกลับจนถึง `ปณ.ต้นทางส่งคืนบริษัท` แต่เดิมระบบดึงเฉพาะสถานะล่าสุด (`events[-1]`) ทำให้ได้เพียงข้อความขั้นตอนการส่งคืน ส่งผลให้การ์ด "สาเหตุการส่งคืน / นำจ่ายไม่สำเร็จ" บนหน้าสถิติไม่สามารถจำแนกสาเหตุที่แท้จริงได้
   - **การแก้ไขใน Backend (`api/index.py`)**:
     - สร้างฟังก์ชัน `_extract_failure_reason_from_events(events)` ค้นหาขั้นตอน `ปณ.ปลายทางส่งคืน` (หรือขั้นตอนส่งคืนแรก) และสกัดเอาสถานะข้อยกเว้นที่เกิดขึ้นก่อนหน้านั้นทันที
     - สร้างฟังก์ชัน `_normalize_failure_reason(raw_text)` ลอกวงเล็บและจำแนกหมวดหมู่สถานะทางการของไปรษณีย์ไทยอย่างแม่นยำ เช่น `ย้าย / ไม่ทราบที่อยู่ใหม่`, `บ้านปิด`, `ออกใบแจ้ง`, `ติดต่อผู้รับไม่ได้`
     - ใน `POST /api/reports/dashboard`: ดึง `getHistoryStatus` สำหรับรายการพัสดุที่ส่งคืน/ไม่สำเร็จทั้งหมดแบบขนานผ่าน `ThreadPoolExecutor` แล้วสกัดสาเหตุที่แท้จริงก่อนขั้นตอนส่งคืนบันทึกลงใน `failure_reason` และรวบรวมเป็นสถิติสรุปบนการ์ดอย่างถูกต้อง
   - **การแก้ไขใน Frontend (`TrackingTimelineModal.jsx`)**:
     - เพิ่มฟังก์ชัน `extractFailureReason` ค้นหาสถานะก่อน `ปณ.ปลายทางส่งคืน` และปรับแต่งป้าย Badge สถานะด้านบนขวาให้แสดง `นำจ่ายไม่สำเร็จ (ย้าย / ไม่ทราบที่อยู่ใหม่)` หรือข้อยกเว้นจริงอย่างชัดเจน

### 17.2 ผลลัพธ์การทดสอบและการ Build
- ตรวจสอบความถูกต้องของสคริปต์ Python AST: ผ่าน 100%
- ทดสอบระบบป้องกันสระวรรณยุกต์และตัวอักษรผิดเพี้ยน (`encoding_guard.mjs`): ผ่าน 56 ไฟล์ 100% (0 mojibake)
- การคอมไพล์ Production Bundle (`npm run build`): สำเร็จไร้ข้อผิดพลาด
...
## 19. ปรับสัดส่วนความกว้างการ์ดแผนที่และการ์ดจัดอันดับจังหวัดให้เท่ากัน 50/50 (`v2026.0924.2025`)

### 19.1 รายการแก้ไขและปรับปรุง
1. **บั๊ก #50 (Asymmetric Dashboard Map Grid) — การ์ดแผนที่ประเทศไทยกับการ์ดจัดอันดับจังหวัดมีความกว้างไม่เท่ากัน (`DashboardView.css`)**:
   - **รากเหง้าของปัญหา**: ในไฟล์ `DashboardView.css` กำหนด `.dash-map-layout` เป็น `grid-template-columns: minmax(0, 1.35fr) minmax(0, 1fr);` ส่งผลให้การ์ดแผนที่ทางฝั่งซ้ายขยายกว้างถึง ~57% ในขณะที่การ์ดจัดอันดับจังหวัดทางฝั่งขวาแคบเพียง ~43%
   - **การแก้ไขใน CSS (`DashboardView.css`)**:
     - ปรับ `.dash-map-layout` ให้ใช้ `grid-template-columns: repeat(2, minmax(0, 1fr));` ส่งผลให้การ์ดทั้งสองใบมีความกว้างเท่ากัน 50% / 50% อย่างสมบูรณ์
     - กำหนด `min-width: 0; width: 100%;` สำหรับ `.dash-map-card` และ `.dash-ranking-card` เพื่อรองรับการย่อขยายเนื้อหาภายใน

### 19.2 ผลลัพธ์การทดสอบและการ Build
- ตรวจสอบความถูกต้องของสคริปต์ Python AST: ผ่าน 100%
- ทดสอบระบบป้องกันสระวรรณยุกต์และตัวอักษรผิดเพี้ยน (`encoding_guard.mjs`): ผ่าน 56 ไฟล์ 100% (0 mojibake)
- การคอมไพล์ Production Bundle (`npm run build`): สำเร็จไร้ข้อผิดพลาด
- ซิงก์เลขเวอร์ชัน 6 ตำแหน่งตรงกัน: `v2026.0924.2025`
...
## 22. การเพิ่มการ์ด "ยอดรวมค่าบริการ" ในหน้าสถิติ และการแก้ไขสัดส่วนเปอร์เซ็นต์สาเหตุการส่งคืน/นำจ่ายไม่สำเร็จ (v2026.0924.2340)

### 📌 ปัญหาที่พบ
1. ในหน้าจอแดชบอร์ดสถิติ ("สถิติ") มีการ์ดสรุปสถานะเพียง 5 ใบ ขาดการ์ด **"ยอดรวมค่าบริการ"** ซึ่งผู้ใช้ต้องการให้มีเหมือนกับหน้ารายงานสถานะ ("รายงานสถานะ")
2. ในการ์ด **"สาเหตุการส่งคืน / นำจ่ายไม่สำเร็จ"** เกิดการแสดงผลเปอร์เซ็นต์ที่ผิดปกติอย่างมาก เช่น "บ้านปิด 9 รายการ 450.00%" และ "ย้าย / ไม่ทราบที่อยู่ใหม่ 2 รายการ 100.00%" พร้อมแถบ Progress Bar สีแดงเต็ม 100% ทั้งคู่ ทำให้ผู้ใช้เกิดข้อสงสัยว่าทำไมเปอร์เซ็นต์จึงเกิน 100%

### 🔍 สาเหตุรากเหง้า (Root Cause)
1. **ขาดการ์ดและข้อมูลค่าบริการในแดชบอร์ด**:
   - `DashboardView.jsx` ยังไม่ได้กำหนดการ์ดใบที่ 6 ในอาร์เรย์ `statCards`
   - ใน `api/index.py` ฟังก์ชัน `get_dashboard_report` ยังไม่ได้ส่งตัวแปร `total_fee` ออกไปในออบเจกต์ `summary`
2. **สูตรคำนวณเปอร์เซ็นต์สาเหตุอ้างอิงตัวหารผิด**:
   - ใน `api/index.py` โค้ดเดิมคำนวณ `pct_of_failed = c * 100 / (failed if failed else total_reasons)`
   - เมื่อสถานะ "บ้านปิด" (9 รายการ) ถูกจัดหมวดเป็น `in_transit` ทำให้ยอดพัสดุตีกลับจริง (`failed`) เหลือเพียง 2 รายการ (ย้าย/ไม่ทราบที่อยู่ใหม่)
   - ส่งผลให้ระบบนำ 9 ไปหารด้วย 2 (`9 * 100 / 2 = 450.00%`) และ 2 ไปหารด้วย 2 (`2 * 100 / 2 = 100.00%`) แทนที่จะหารด้วยจำนวนสาเหตุทั้งหมดที่พบ (11 รายการ)

### 🛠️ การแก้ไขที่ดำเนินการ
1. **การ์ด "ยอดรวมค่าบริการ" ใน `DashboardView.jsx` & `api/index.py`**:
   - ใน `api/index.py` ดึง `total_fee` จาก `payload["summary"]["total_fee"]` หรือคำนวณจากผลรวมค่าธรรมเนียมของรายการพัสดุ แล้วส่งกลับใน `summary["total_fee"]`
   - ใน `DashboardView.jsx` เพิ่มการ์ดใบที่ 6 ใน `statCards` ใช้ไอคอน `icon-fee`, ตัวเลขสีทองประกาย `text-gold` พร้อมจัดรูปแบบจุลภาคและทศนิยม 2 ตำแหน่ง (`toLocaleString('th-TH')`)
   - ใน `DashboardView.css` เพิ่มคลาส `.stat-card-icon.icon-fee` และ `.text-gold` รองรับทั้ง Light/Dark Mode
2. **แก้ไขการคำนวณสัดส่วนเปอร์เซ็นต์สาเหตุ**:
   - ใน `api/index.py` ปรับสูตรให้หารด้วย `total_reasons` (ผลรวมสาเหตุทั้งหมด):  
     `pct = round(c * 100 / total_reasons, 2) if total_reasons else 0.0`
   - ใน `DashboardView.jsx` ปรับการคำนวณในลูปแสดงผล:  
     `const pct = totalReasonsCount > 0 ? (r.count * 100 / totalReasonsCount) : (Number(r.pct ?? r.pct_of_failed) || 0);`
   - ผลลัพธ์: "บ้านปิด" (9 รายการ) แสดง `81.82%` และ "ย้าย / ไม่ทราบที่อยู่ใหม่" (2 รายการ) แสดง `18.18%` รวมกันได้ 100% พอดี พร้อมแถบ Progress Bar กว้างตามสัดส่วนที่ถูกต้อง
...
## 25. แยกการ์ดแสดงผล "สาเหตุการส่งคืน" และ "สาเหตุการนำจ่ายไม่สำเร็จ" ในหน้าสถิติ (v2026.0925.0215)

### 📌 ปัญหาและความต้องการของผู้ใช้
- ผู้ใช้งานแจ้งความต้องการ: **"ต้องการให้แยกการ์ดการแสดงออกจากกันว่า 'สาเหตุการส่งคืน' มีอะไรบ้าง และ 'นำจ่ายไม่สำเร็จ' มีอะไรบ้าง"**
- เดิมระบบนำสาเหตุของพัสดุที่ถูกส่งคืน (เช่น `ย้าย / ไม่ทราบที่อยู่ใหม่`) และสาเหตุของพัสดุที่นำจ่ายรอบแรกไม่สำเร็จแต่อยู่ระหว่างนำจ่าย (เช่น `บ้านปิด`) มารวมอยู่ในการ์ดเดียวกันภายใต้หัวข้อ "สาเหตุการส่งคืน / นำจ่ายไม่สำเร็จ" ทำให้ผู้ใช้ไม่สามารถแยกแยะได้ชัดเจนว่ารายการใดคือพัสดุที่ตีกลับจริง และรายการใดคือเหตุขัดข้องในการนำจ่ายชั่วคราวระหว่างทาง

### 🔍 สาเหตุและการออกแบบแก้ปัญหา
1. **การจำแนกประเภทสาเหตุใน Backend (`api/index.py`)**:
   - พัสดุที่มีเหตุขัดข้องในการนำจ่ายมี 2 รูปแบบหลัก:
     1. **ส่งคืนต้นทาง (Returned)**: พัสดุสิ้นสุดการนำจ่ายและส่งคืนผู้ฝาก (`is_actual_return` / `key == "returned"`)
     2. **นำจ่ายไม่สำเร็จชั่วคราว (Attempt Failed)**: พัสดุที่มีเหตุขัดข้องในการนำจ่ายรอบแรก เช่น "บ้านปิด", "ออกใบแจ้ง" ซึ่งยังอยู่ระหว่างนำจ่าย (`key == "in_transit"`)
   - เดิมฟังก์ชัน `get_dashboard_report` รวมทั้งสองกลุ่มเข้าใน `reason_counts` เดียวกัน ทำให้ไม่สามารถจำแนกสัดส่วนและประเภทออกจากกันได้
2. **การแยกการ์ดและการจัดวาง (Frontend UI/UX)**:
   - นำเสนอการ์ด 2 ใบแบบคู่ขนาน (Twin Analytical Cards) ด้วย Responsive Grid (`.dash-reasons-container`):
     - **การ์ด 1: สาเหตุการส่งคืน** (โทนสีแดง/กุหลาบ `#e11d48`, ไอคอนลูกศรส่งคืน `icon-returned`)
     - **การ์ด 2: สาเหตุการนำจ่ายไม่สำเร็จ** (โทนสีส้ม/อำพัน `#f59e0b`, ไอคอนแจ้งเตือน `icon-transit`)
   - คำนวณเปอร์เซ็นต์ของแต่ละกลุ่มแยกเป็นเอกเทศจากกัน (100% ของสาเหตุส่งคืน และ 100% ของสาเหตุนำจ่ายไม่สำเร็จ)

### 🛠️ การแก้ไขที่ดำเนินการ
1. **`api/index.py` (`get_dashboard_report`)**:
   - เพิ่มตัวแปร `return_reason_counts` และ `delivery_failed_reason_counts`
   - ในลูปประมวลผลพัสดุ จำแนก `reason_type` เป็น `"return"` หรือ `"delivery_failed"`
   - ส่งออกอาร์เรย์ `return_reasons` และ `delivery_failed_reasons` พร้อมค่า `pct` ที่คำนวณแยกตามกลุ่ม
   - ส่งออก `total_return_reasons` และ `total_delivery_failed_reasons` ใน `summary`
2. **`src/components/DashboardView.jsx`**:
   - เพิ่ม `useMemo` คำนวณ `returnReasons` และ `deliveryFailedReasons` พร้อม Fallback รองรับข้อมูลแคชเดิม
   - แยกการ์ดเดิมออกเป็น 2 การ์ด: `dash-return-card` และ `dash-failed-card`
   - แสดงหัวข้อและ Subtitle สรุปจำนวนรายการและจำนวนพัสดุของแต่ละกลุ่มชัดเจน
3. **`src/components/DashboardView.css`**:
   - เพิ่ม `.dash-reasons-container` จัดวาง Grid 2 คอลัมน์บนจอคอมพิวเตอร์ และ 1 คอลัมน์บนจอมือถือ
   - เพิ่มสไตล์เฉพาะกลุ่ม: `.dot-return`, `.chip-return`, `.bar-return`, `.dot-failed`, `.chip-failed`, `.bar-failed`
   - ปรับ Hover Effect ให้เรืองแสงตามโทนสีของแต่ละกลุ่ม

---

## 26. ซ่อนแถบแบนเนอร์ล็อคข้อมูลจังหวัดในการ์ดรายละเอียด (v2026.0925.0635)

### 📌 ปัญหาและความต้องการของผู้ใช้
- ผู้ใช้งานส่งรูปแบนเนอร์สีทอง `[ 🔒 ล็อคข้อมูลจังหวัด   ปลดล็อค ✕ ]` และแจ้งความต้องการ: **"ซ่อนอันนี้ ไม่ต้องแสดง"**
- เดิมเมื่อคลิกล็อคจังหวัดบนแผนที่ จะมีแถบแบนเนอร์สีทองปรากฏด้านบนของการ์ดรายละเอียด ทำให้ตัวการ์ดกินพื้นที่แนวตั้งและดูรกสายตา

### 🔍 สาเหตุและการออกแบบแก้ปัญหา
- แถบ `.dash-pd-lock-banner` ถูกออกแบบไว้เดิมเพื่อให้ผู้ใช้ทราบสถานะและมีปุ่มปลดล็อค แต่ส่งผลให้การ์ดไม่เรียบหรูและกินพื้นที่
- การปลดล็อคสามารถกระทำได้ง่ายๆ โดยการคลิกซ้ำที่จังหวัดเดิมบนแผนที่ หรือคลิกแถวเดิมในตารางจัดอันดับจังหวัดอยู่แล้ว
- จึงนำแถบแบนเนอร์ออกทั้งหมด และใส่ปุ่มปิด `✕` ขนาดเล็กกะทัดรัดไว้ที่มุมขวาบนของแถวชื่อจังหวัดเพื่อเป็นทางเลือกในการปลดล็อค 1-คลิก

### 🛠️ การแก้ไขที่ดำเนินการ
1. **`src/components/DashboardView.jsx`**:
   - ลบ `.dash-pd-lock-banner` และ `.dash-pd-hover-hint` ออกจากการ์ดรายละเอียดจังหวัด
   - เพิ่มปุ่มปิด `✕` (`.dash-pd-close-btn`) ต่อท้ายป้ายเปอร์เซ็นต์ใน `.dash-pd-rate-wrap` แสดงเฉพาะเมื่อจังหวัดถูกล็อค
2. **`src/components/DashboardView.css`**:
   - เพิ่มคลาส `.dash-pd-rate-wrap` และ `.dash-pd-close-btn` ดีไซน์ Clean Minimalist รองรับ Light/Dark Mode
